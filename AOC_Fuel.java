...
public class AOC_Fuel {
    public static void main(String[] args) {
        ...
        int mean = (int)Math.ceil(sum/(double)arr.length);
        int median = kthSmallest(arr, 0,arr.length-1, arr.length/2);
        long fuel = 0;
        long min = Long.MAX_VALUE;
        HashMap<Integer, Long> hm = new HashMap<Integer, Long>();
        hm.put(1,Long.valueOf(1));
        hm.put(0,Long.valueOf(0));
        for(int i=median; i<=mean; i++) {
            for(int n:arr) {
    ...
    public static long helper(int key, HashMap<Integer, Long> hm) {
        if(!hm.containsKey(key)){
            hm.put(key, helper(key-1, hm)+key);
        }
        return hm.get(key);
    }
